...
|---|---|---|---|
| `raw_news` | Original fetched news data | `fetch` | `clean`, `export` |
...
- **이창민(조장)**: 전체 진행, 설계, 통합, 발표 총괄
- **김용덕**: 데이터 수집, 정제, DB 저장
- **구수진**: Gemini API 기반 요약 및 분석
- **김서영**: 시각화, 내보내기, 문서화, 검증
...
| 단계 | 주 담당 | 보조 | 주요 내용 |
|---|---|---|---|
| 진행 | 이창민 | 전원 | 일정 관리, 회의 진행, Git 작업 관리 |
| 설계 | 이창민, 김용덕 | 구수진, 김서영 | DB 구조, CLI 구조, 데이터 흐름 설계 |
| 구현 | 김용덕, 구수진, 김서영 | 이창민 | 기능별 모듈 구현 |
| 통합 | 이창민 | 김용덕, 구수진, 김서영 | 전체 CLI 연결 및 실행 흐름 점검 |
| 검사 | 김서영 | 이창민, 김용덕, 구수진 | 테스트, 체크리스트 확인, 실행 결과 검증 |
| 문서화 | 김서영 | 이창민 | README, TASK, CHECKLIST 정리 |
| 발표 | 이창민 | 김용덕, 구수진, 김서영 | 발표 흐름 구성 및 파트별 설명 |
